...
Video::Video()
	: love::video::Video("love.video.mp4")
{
...
{
	MP4VideoStream *stream = new MP4VideoStream(file);
	workerThread->addStream(stream);
	return stream;
}

bool Video::canDecode(love::filesystem::File *file)
{
	return MP4VideoStream::isSupported(file);
}

Worker::Worker()
	: stopping(false)
{
	threadName = "MP4VideoWorker";
}
...
void Worker::addStream(MP4VideoStream *stream)
{
